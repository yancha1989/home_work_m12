function ElecDevice(name, firm) {
  (this.name = name), (this.firm = firm), (this.type = "electric");
}
...
function KitchenDevice(name, purpose) {
  (this.name = name), (this.purpose = purpose);
}

KitchenDevice.prototype = new ElecDevice();

function BeautyDevice(name, firm, purpose) {
  (this.name = name), (this.firm = firm), (this.purpose = purpose);
}

BeautyDevice.prototype = new ElecDevice();

KitchenDevice.prototype.getPower = function eLectr(
  name,
  turnOn,
  passsport_power
) {
  ...
  if (turnOn === "turnOn") {
    result =
      "Если использовать " +
      name +
      " три раза за день, то потребляемая мощность составит " +
      passsport_power * (1 / 3) +
      " Ватт";
  } else {
    result = "Прибор выключен";
